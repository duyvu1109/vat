...
export default function TempGauge(props) {
        ...
        if (props.attribute) {
            if (props.attribute.color) {
                ...
                if (newArrayColor.length === 0) {
                    setArrayColor(['#FF0000', '#FF3300', '#FFFF00', '#CCFFCC', '#3366CC', '#0099CC', '#3366CC']);
                } else {
    ...
    return (
        <GaugeChart
            id="gauge-chart3"
            nrOfLevels={attribute ? attribute.number_of_element : 30}
            colors={arrayColor}
            ...
            formatTextValue={() => value.toFixed(1) + (attribute ? ' ' + attribute.signature : ' ℃')}
